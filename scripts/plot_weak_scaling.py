...
def plot_weak_scaling(results, output_path):
    ...
    # Colors and markers for periodic and non-periodic (grey-blue-purple range)
    colors = {'Non-Periodic': '#666666', 'Periodic': '#6a4c93'}  # Grey and purple
    ...
    # Plot single ideal runtime line (ideal weak scaling: runtime remains constant)
    # Use the non-periodic baseline as the ideal reference
    if 'Non-Periodic' in results:
        baseline0 = results['Non-Periodic']['baseline_runtime']
        ax_runtime.axhline(y=baseline0, color='k', linestyle='--', 
                          linewidth=1.5, alpha=0.5, label='Ideal', zorder=1)
