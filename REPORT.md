...
Non-contiguous data (e.g., vertical columns of the grid) must be packed into contiguous buffers before sending. Initially, this was a serial bottleneck. We parallelized the packing and unpacking loops using OpenMP. This simple optimization provided a 3-4× speedup in the communication preparation phase, further reducing the overhead exposed to the critical path.
